...
OpenDeepHole 中所有模型任务统一调用 `agent.opencode.run_opencode_task()`。业务组件不直接启动 CLI，不访问任务队列，也不自行创建、查询、取消或删除 OpenCode Session。
...
2. `OPENCODE_TASK_CONFIG` 环境变量；
3. 当前目录的 `opencode-agent.yaml`。

配置格式见 `agent/opencode/opencode-agent.example.yaml`。`context` 必须给出项目目录、固定可写工作目录和 OpenCode workspace；`serve` 给出工具、可执行文件、端口、环境变量和完整 OpenCode 配置对象；`model_pool` 至少包含一个已启用的明确模型或显式默认模型行。相对路径按 YAML 所在目录解析，工作目录和 workspace 自动创建。
...
- `agent/opencode/api.py`：唯一公共调用与精简结果契约。
- `agent/opencode/task_service.py`：内部队列、模型调度、权限、Session、JSON 纠正和重试。
- `agent/opencode/model_pool.py`：模型 Lease、全局并发、能力匹配、时间窗和统计。
- `agent/opencode/serve_client.py`：Serve 生命周期、Session API、事件与消息流。
- `agent/opencode/host.py`：自包含组件与宿主之间的最小配置回调边界。
- `agent/opencode/standalone.py`：独立 YAML 的校验、发现、宿主适配和一次性自举。
...
`agent/opencode/` 内不导入 OpenDeepHole 的 `agent`、`backend`、`mcp_server` 或 `code_parser` 模块；单独复制该目录时，安装 `httpx` 与 `PyYAML` 并提供独立配置即可运行，也可由其它应用注册自己的 `OpenCodeHostBindings`。OpenDeepHole 业务阶段只能从 `agent.opencode` 导入公共类型和函数，不应直接依赖 `task_service.py` 中的内部任务记录、句柄或 Session 管理方法。
