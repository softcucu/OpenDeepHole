...
- 如果 `dstsz` 等于该 `dst` 的真实可写空间，直接按本 checker 的误报处理，不需要继续追 `count` 是否大于容量
...
只有在 Step 2 已经证明 `dstsz` 大于真实可写空间时，才需要继续分析 `count`、源字符串长度或格式化输出长度是否会越界。
如果 `dstsz` 与真实缓冲区大小一致，即使 `count` 更大，也会被安全函数的容量校验拦住，不应确认为 `safe_mem_oob`。
...
- 如果 `dstsz` 已经等于真实可写空间，不需要再用 `count` 确认 OOB
...
- 申请长度是多少
- 真实可写空间是多少
- `dstsz` 是否等于真实可写空间；如果相等，当前 checker 可提前判为误报
- 这次调用实际会写多少
- 写入前有没有边界检查
- 边界检查是否覆盖所有路径
...
- `dstsz` 大于该 `dst` 的真实可写空间；如果 `dstsz` 等于真实可写空间，不确认本 checker 的 OOB
...
- `dstsz` 与真实缓冲区大小一致，即使 `count` 可能更大，也会被安全函数按正确容量拦住
- 代码位于测试、mock、stub 或不可达分支
- 项目封装函数的参数顺序与规则假设不一致，无法证明适用

## 重点反误报规则

### 1. `dstsz` 等于真实缓冲区大小时不要继续看 `count`

如果 `dstsz` 与目标缓冲区真实大小一致，或与偏移后的真实剩余可写空间一致，当前调用不属于 `safe_mem_oob`。

这时即使 `count`、源字符串长度或格式化输出长度更大，也会被安全函数用正确的 `dstsz` 校验住，最多表现为函数失败、截断或返回错误，不会写出目标边界。

返回值未检查可能是其他逻辑问题，但不要在本 checker 中确认为越界写。

### 2. `sizeof(pointer)` 不要单独确认
...
### 3. `dstsz == count` / `dstsz == srcsz` 不要单独确认
