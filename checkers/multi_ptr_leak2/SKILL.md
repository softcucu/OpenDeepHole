...
description: 验证结构体最外层指针释放时是否遗漏释放其指针成员，判断 multi_ptr_leak2 静态候选是否为真实资源泄露
---

# 多层指针外层释放遗漏成员验证

你正在验证一个 `multi_ptr_leak2` 静态候选。静态分析器已经用 tree-sitter 做过第一轮过滤：候选位置存在释放函数调用，并且**某一个表达式**已解析为含指针成员的结构体/类/联合体对象。你的任务是判断释放路径是否只释放了最外层对象，而没有释放结构体内部应由它拥有的指针成员。
...
- `静态分析实参: first_argument | receiver | delete_operand` —— ⚠️ 静态阶段
  解析为"含指针成员结构体"的那个表达式，不一定是第一个显式参数。method
  调用下可能是 receiver（例如 `obj->destroy()`）
- `receiver: <expr>`（仅 method_call）—— receiver 表达式文本
- `释放调用命中关键字: <keyword>` —— callee 名字命中了哪个释放关键字
  （callee 语义提示，不是函数被认定为释放函数的原因）
- `释放调用: <callee>(<arg_text>)`
- `实参类型 / 结构体 / 指针成员 / 调用点上下文`

特别注意 `静态分析实参`：如果是 `receiver`，意味着候选要审计的对象是
