...
你正在验证一个由 semgrep 静态分析发现的候选双重释放漏洞（CWE-415）。你的任务是判断这是真实的 bug 还是误报。

## 背景

静态分析器已经完成了以下工作：

- 使用 semgrep 扫描了 14 条规则，覆盖 7 大类双重释放模板（A: 错误路径释放 + 公共 cleanup 再释放；C: 成员释放与 owner cleanup 重叠；D: 所有权转移 API 后 caller 再释放；E: refcount get/put 失衡；G: C++ 浅拷贝 + 析构释放裸指针；H: 智能指针重复包装裸指针 / 手动释放 get() 结果；I: new/delete/malloc/free 配对错位）
- 在候选位置发现了"同一资源 `$P` 或 `$OBJ->$FIELD` 出现在两次释放调用之间，且中间没有 `= NULL` 置空"的模式
- candidate 描述中的**规则类型**指明了具体是哪类模板，**指针/资源、宿主对象、成员字段、释放调用、所有权 API** 等元信息已尽量提取

semgrep 是纯语法模式匹配，无法感知：跨函数的所有权转移语义、释放包装函数内部是否真的释放、release 后但 goto 之前是否 `p = NULL`、refcount get 实际调用次数、宏内的 NULL 置位、跨编译单元的析构语义。你需要做语义层面的验证。
...
用 `view_function_code` 获取 candidate 所在函数的完整源码。candidate 描述只包含 semgrep 匹配的几行片段，无法判断两次释放之间是否存在 `= NULL` 置位、是否存在 `return` 跳出、是否存在条件守卫。**必须**先看到整个函数。

### Step 2 — 查看释放函数的语义

candidate 描述中的"释放调用 / refcount put / 所有权 API"都是名称模式匹配（free/destroy/release/unref/put/take/add/register 等正则）。这些名字**不一定**等于"真的释放内存"：
...
| 规则类型 | 核心验证问题 |
|---------|------------|
| `A.release-before-goto-cleanup-same-resource` | 第一次 `$REL1($P)` 之后 / `goto $LABEL` 之前，`$P` 是否被置 NULL？`$REL2` 是否能容忍 NULL？两次释放是否真的释放同一片内存？ |
| `A.delete-before-goto-cleanup-same-resource` (C++) | 同上，C++ `delete` 不容忍重复（即便 `delete nullptr` 安全，但 `delete` 一个非 NULL 已释放指针就是 UB） |
| `C.member-release-before-owner-cleanup` | `$OBJ->$FIELD` 被释放后，`$REL2($OBJ)` 内部是否也会释放该成员？该 owner cleanup 是否依赖 `$OBJ->$FIELD == NULL` 才跳过？ |
| `C.member-delete-before-owner-cleanup` (C++) | 同上 |
| `D.possible-ownership-transfer-then-caller-release` | `$TAKE` 是否真的接管 `$P` 的所有权？是"成功时接管，失败时不接管"还是"无论如何都接管"？后续 `$REL($P)` 是否在 `$TAKE` 成功后执行？ |
| `D.transfer-error-branch-release` | `$TAKE` 失败时（`$RC < 0` / `!$RC`），它是否在内部已经释放了 `$P`？常见坑：调用约定模糊导致"调用方也释放" |
| `E.refcount-put-before-goto-cleanup-put` | `$GET` 实际增加了几次 refcount？`$PUT1` 之后 refcount 是否仍 > 0？`$PUT2` 是否对应另一次 get？ |
| `G.raw-pointer-owner-destructor-rule-of-five` | 该类是否定义了拷贝构造 / 拷贝赋值 / 移动构造 / 移动赋值（Rule of Five）？是否声明 `= delete` 禁止拷贝？类对象是否真的会被拷贝（按值传递、放入 STL 容器、赋值）？ |
| `H.smart-pointer-duplicate-raw-ownership` | 两个智能指针构造时用的是同一个 `$P`？若都是 `shared_ptr`，会产生两个独立 control block 各自 delete；若是 `unique_ptr`，明确双重所有权 |
| `H.smart-pointer-get-manual-release` | 是否对 `owner.get()` 的返回值做了 delete / free？smart_ptr 析构时是否还会释放一次？是否本意是 `owner.release()`？ |
| `I.new-pointer-released-by-free-like-function` | `new` 出来的对象用 `free()`/wrapper 释放：违反 C/C++ allocator/deallocator 配对；具体运行时是否真的崩溃因 allocator 而异，但属于 UB |
| `I.new-array-deleted-as-scalar` | `new T[N]` 必须 `delete[]`，scalar `delete` 是 UB |
| `I.new-scalar-deleted-as-array` | `new T` 必须 `delete`，`delete[]` 是 UB |
| `I.malloc-like-pointer-deleted` | `malloc/calloc/strdup/kmalloc/...` 必须用对应的 `free/kfree/g_free/...` 释放，不能用 C++ `delete` |

### Step 4 — 验证"看不见的"置空 / 返回 / 守卫

semgrep 的 `pattern-not` 只能匹配字面 `$P = NULL`，会漏掉很多形式的"已置空"：
...
- **早 return**：第一次释放后是否 `return` 跳出，根本到不了第二次释放？semgrep 的 `goto` 模板理论上能识别 `goto $LABEL`，但若实际控制流是 `return` 或 `break` 跳出，告警可能本身就是误报。
...
- `ai_analysis`：详细推理，需包含：
  ...
  3. 两次释放之间是否存在 NULL 置位 / return / 守卫，semgrep 为何遗漏（如有）
