"""双重释放静态分析器 — 使用 semgrep 扫描 CWE-415 模式。
...
from deephole_client.static_analysis.semgrep_locations import function_from_db_location
...
_MESSAGE_FUNCTION_RE = re.compile(r"Function\s+([A-Za-z_][A-Za-z0-9_:]*)")
...
class Analyzer(BaseAnalyzer):
    ...
    def find_candidates(
        ...
        if not shutil.which("semgrep"):
            _log.warning("semgrep not found; double_free checker skipped")
            return
        ...
        # 去重键：文件 + 函数 + 规则 + 指针/对象/字段 + 释放函数
        # 同函数内对不同指针 / 不同 owner 的告警仍保留
        ...
        for match in data.get("results", []):
            ...
            obj_name = _mv(metavars, "$OBJ")
            field_name = _mv(metavars, "$FIELD")
            rel1 = _mv(metavars, "$REL1")
            rel2 = _mv(metavars, "$REL2")
            rel = _mv(metavars, "$REL")
            put1 = _mv(metavars, "$PUT1")
            put2 = _mv(metavars, "$PUT2")
            get_fn = _mv(metavars, "$GET")
            take_fn = _mv(metavars, "$TAKE")
            label = _mv(metavars, "$LABEL")
            class_name = _mv(metavars, "$CLASS")
            type_name = _mv(metavars, "$TYPE")
            sp1 = _mv(metavars, "$SP1")
            sp2 = _mv(metavars, "$SP2")
            owner = _mv(metavars, "$OWNER")
            alloc = _mv(metavars, "$ALLOC")

            # 去重时的"资源标识"：尽量唯一标识这次告警的目标资源
            resource_key = ptr_name or (
                f"{obj_name}.{field_name}" if obj_name or field_name else ""
            )
            release_key = (
                rel1 or rel2 or rel or put1 or put2 or alloc or ""
            )
            dedup_key = (
                rel_path, func_name, rule_category,
                class_name or type_name,
                resource_key,
                release_key,
            )
            ...
            details: list[str] = []
            if ptr_name:
                details.append(f"指针/资源: {ptr_name}")
            if obj_name:
                details.append(f"宿主对象: {obj_name}")
            if field_name:
                details.append(f"成员字段: {field_name}")
            if class_name:
                details.append(f"所在类: {class_name}")
            if type_name:
                details.append(f"类型: {type_name}")
            if rel1 or rel2:
                pair = " / ".join(x for x in (rel1, rel2) if x)
                if pair:
                    details.append(f"释放调用: {pair}")
            if rel:
                details.append(f"释放调用: {rel}")
            if put1 or put2:
                pair = " / ".join(x for x in (put1, put2) if x)
                if pair:
                    details.append(f"refcount put: {pair}")
            if get_fn:
                details.append(f"refcount get: {get_fn}")
            if take_fn:
                details.append(f"所有权转移 API: {take_fn}")
            if alloc:
                details.append(f"分配器: {alloc}")
            if sp1 or sp2:
                pair = " / ".join(x for x in (sp1, sp2) if x)
                if pair:
                    details.append(f"智能指针: {pair}")
            if owner:
                details.append(f"smart_ptr owner: {owner}")
            if label:
