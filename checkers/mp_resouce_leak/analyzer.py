"""多层指针成员资源泄露静态分析器 — 使用 semgrep 扫描 CWE-401 / CWE-772 模式。
...
_RULE_FILE = Path(__file__).parent / "mp_res_leak.yaml"
...
# 规则消息形如 "多层指针成员 `$FIELD` ..."；函数名不出现在 message 中，
# 此正则作为兜底（极少命中），主要靠 tree-sitter / CodeDatabase 反查。
...
def _field_from_lines(matched_lines: str) -> str:
    """从匹配代码片段里提取多层指针成员表达式（兜底，社区版 $FIELD metavar 为空时使用）。

    形如 `ctx->session->buf = VOS_Malloc(...);` 取赋值号左侧。
    """
    if not matched_lines:
        return ""
    first = matched_lines.splitlines()[0].strip()
    if "=" in first:
...
class Analyzer(BaseAnalyzer):
    ...
    def find_candidates(
        ...
        if not shutil.which("semgrep"):
            _log.warning("semgrep not found; mp_resouce_leak checker skipped")
            return
        ...
        # 去重键：文件 + 函数 + 规则 + 字段表达式 + 资源调用名
        # 同函数对不同 field 或不同 acquire 调用仍保留独立告警
        ...
        for idx, match in enumerate(matches, 1):
            ...
            func_name = _clean_func_name(_mv(metavars, "$FUNC")) or _func_from_message(message)
            ...
            field_expr = _mv(metavars, "$FIELD") or _field_from_lines(matched_lines)
            root = _mv(metavars, "$ROOT")
            acq = _mv(metavars, "$ACQ")
            acq1 = _mv(metavars, "$ACQ1")
            acq2 = _mv(metavars, "$ACQ2")
            realloc_fn = _mv(metavars, "$REALLOC")
            tmp_var = _mv(metavars, "$TMP")
            type_name = _mv(metavars, "$T")

            # 用于去重的资源调用：5 个规则中可能出现的获取/重分配函数
            acquire_key = acq or acq1 or realloc_fn or tmp_var or ""
            ...
            details: list[str] = []
            if field_expr:
                details.append(f"多层成员: {field_expr}")
            if root:
                details.append(f"根对象: {root}")
            if acq:
                details.append(f"资源获取: {acq}")
            if acq1 or acq2:
                pair = " / ".join(x for x in (acq1, acq2) if x)
                if pair:
                    details.append(f"两次获取: {pair}")
            if realloc_fn:
                details.append(f"重分配函数: {realloc_fn}")
            if tmp_var:
                details.append(f"临时变量: {tmp_var}")
            if type_name:
