...
**三阶段 AI 去误报**：FP 复核按 `prove-bug`、`prove-fp`、`final-judge` 顺序运行，各阶段通过本地 Markdown artifact 文件交接；阶段结束后页面即可通过按钮查看该阶段论证，最终结论由 `final-judge` 提交。
...
6. AI 去误报复核会依次运行 `prove-bug`、`prove-fp`、`final-judge` 三个阶段；前两个阶段将 Markdown 写入本次复核的 artifact 目录，后续阶段按文件路径读取，避免把完整论证塞进 prompt
7. 每个阶段结束后，扫描详情页会实时展示对应 Markdown；最终漏洞/误报结论和问题报告只采用 `final-judge` 的 `submit_result`
