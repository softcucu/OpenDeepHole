...
- **优化** `memleak` checker 改为 opencode 复核，并将静态阶段升级为 tree-sitter 路径分析：`if` then/else 分支单独传播，过滤判空宏、参数/参数成员所有权转移和循环内正常释放后的函数尾部 return 误报，同时保留循环 `continue` 与初始化待确认的泄漏候选
- **新增** 扫描详情页已人工标记的问题支持单条或批量取消标记；取消后会删除该标记生成的反馈经验、从本次扫描选中经验中移除，并让该问题重新进入 AI 去误报复核候选
- **优化** AI 去误报复核改为 `prove-bug` / `prove-fp` 正反两阶段：正方先证明问题成立，若正方判定非问题则短路不运行反方；正方证明问题后再由反方证明误报
...
- **优化** 扫描详情页 FP 复核后默认展示问题报告，正反两阶段论证过程改为点击按钮后展开，避免默认占用报告展示区域
