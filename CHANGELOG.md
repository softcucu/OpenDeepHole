...
- **重构** `run_opencode_task()` 在没有后端宿主绑定时可从独立 `task-agent.yaml` 自举项目/工作上下文、Serve 和模型池配置，后端环境继续以宿主配置为唯一来源；独立配置随单例锁定并在 `shutdown_opencode()` 后才允许切换
...
- **重构** OpenCode 的任务队列、模型调度、Session、权限、JSON 纠错和 `opencode/nga serve` 生命周期从后端迁入自包含的 `agent/task_agent/` 组件；业务阶段统一只调用 `agent.task_agent.run_opencode_task()`，首次任务会惰性创建并启动共享 Serve 单例，后续任务继续复用现有的重启、恢复和 Session 机制，不新增独立 CLI
...
- **新增** Agent 模型配置恢复“添加时间段 + 起止时间选择器”交互，每段可独立多选周一至周日并支持同一模型配置多段；调度按 Agent 本地日期和时间判断，跨夜窗口按当前星期生效，旧的无星期时间窗口继续兼容为每天可用
- **新增** 独立「Agent 配置」页面：服务端按 `机器名 + IP` 稳定识别并持久化在线/离线 Agent 配置，页面按基础配置、模型配置、威胁分析、代码图谱、产品信息、漏洞挖掘、去误报和漏洞验证八个维度编辑；基础配置仅保留工具、工具可执行文件名和代理跳过列表，扫描必须先手动添加至少一个已启用的明确模型
- **新增** 威胁分析、漏洞挖掘和去误报支持阶段级模型能力、调用超时与重试策略，阶段策略统一覆盖具体模型行；代码图谱和产品信息支持本地进程或远程 HTTP MCP 配置，CodeGraph 本地模式会按项目准备索引，可用时替代内置代码 MCP，不可用时继续回退到内置代码索引
- **新增** Agent 配置页的代码图谱与产品信息区域分离“检测连接”和“重新加载”：Agent 直接执行协议握手和工具发现并持久化工具列表、错误和检测时间，同时从 OpenCode `/mcp` 展示正在热加载、已连接、认证失败、加载失败及实际加载目录数；离线仍可查看历史检测结果，检测超时最多 30 秒且不会调用业务工具
- **新增** 远程 MCP 请求头改为结构化名称/值编辑，支持 `Authorization: Bearer ...` 等静态认证头；敏感值默认隐藏，保存时校验非法名称、大小写重复和换行注入，连接检测与 OpenCode 实际加载使用同一组请求头
- **变更** 保存、启用、停用或修改 MCP 配置不要求重启 Agent，也不再等待所有 Session 空闲；Agent 会通过 OpenCode 动态 MCP 接口立即同步到已经使用过的项目目录，不中断正在执行的 Session，并保证后续新建或续用 Session 在工具发现前等待最新配置完成加载；当前没有运行目录时由下一个 Session 自动加载
- **新增** 漏洞验证环境及方法的 `validator.yaml` 可声明支持的漏洞类型、Agent 全局并发、整体验证重试、模型策略和自定义字段；自定义配置会在后端配置页展示并通过 `ctx.<字段>`、`ctx.config.<字段>` 或 `ctx.get_config(...)` 读取，不同环境独立排队和限流
- **变更** 漏洞验证不设置验证函数的整体截止时间；阶段配置中的超时仅约束验证器发起的模型调用，`ctx.run_command(timeout=...)` 继续独立控制每一条命令的超时，整体验证重试也不会改变命令超时
- **变更** 产品漏洞验证使用 `validator.yaml + validator.py` 的独立方法目录，manifest 可严格声明一个或多个产品/验证环境注册及其动态配置字段；创建扫描和历史编辑只允许选择所选 Agent 实际上报的有效配对，产品与环境也可同时留空以关闭验证
...
- **新增** 抽象统一 `OpenCodeTaskService`：任务可指定名称、低/中/高能力、运行目录、MCP 工具、SKILL、prompt、执行超时、`1..100` 优先级、JSON Schema、session 权限和已有 `session_id`；返回 task/session/message、文本结果、本地提取的 JSON、实际模型和输出来源，并支持 session 查询、结果读取、续写、取消与删除
...
- **变更** 确定性任务通过普通回复文本返回 JSON，业务层负责解析、校验和落盘；任务超时只从获得模型后开始计算，session 续写固定运行目录并按 session 串行执行
...
- **变更** 扫描“恢复”和“续扫未完成”合并为统一“续扫”：一次操作会同时继续未扫描的静态候选、重试 `timeout`/`no_result`/`failed` 候选，并继续威胁分析派生的所有未完成审计任务；复用已有威胁分析结果时不再跳过后续威胁审计
- **新增** 扫描历史首页和扫描详情页统一提供“续扫”与“停止”控制；扫描历史的任务进度改为展示同一扫描作用域内 OpenCode/nga 总任务数与已执行数，覆盖候选审计、威胁分析、威胁审计和对抗式去误报等任务
- **新增** 扫描详情首页持久展示 OpenCode/nga 完成任务历史，包括任务类型、目标、模型、时间、耗时、结果、Prompt 长度和当时实际发送的完整 Prompt；扫描完成、停止、Agent 中断或再次续扫后历史和 Prompt 均会保留
...
- **修复** Windows 上 OpenCode/nga serve 事件流若返回 `type=sync`、`name=session.next.*.1` 的事件形态，Agent 控制台现在会正确识别并打印 LLM 文本、推理和工具调用摘要
...
- **新增** 漏洞验证页支持由验证脚本动态创建输出栏：`ctx.emit_stdout("标题", "内容")` 会按标题追加或创建栏位，`ctx.run_command(..., output_title="标题")` 可指定命令输出栏；产物发布新增 `title` 参数并按标题分组展示，单漏洞 Markdown 和扫描报告 ZIP 会导出验证栏与产物内容
- **修复** OpenCode/兼容 CLI 模型池调度纳入计划任务入队顺序；同一扫描候选点审计即使由多个 worker 并发申请租约，也不会让后序候选越过可运行的前序候选，同时保留高能力任务等待时低能力空闲模型可执行后续低能力任务的能力感知调度语义
- **修复** 静态分析统一排除项目目录下 `.opendeephole` 内部运行目录；代码索引、semgrep/cppcheck 和文件遍历型候选生成都不再扫描其中内容，旧索引会因索引器版本变化自动重建
- **修复** 扫描详情页执行流程图中「候选点审计」同时指向「漏洞验证」和「对抗式去误报」；流程图整体居中展示，「漏洞挖掘」分组内节点改为居中紧凑排布，收敛横向留白，分组标题改为与流程节点一致的字号和前景色
