...
- **新增** Agent 配置页的代码图谱与产品信息区域提供手动 MCP 检测及启用状态：Agent 直接执行协议握手和工具发现，展示工具列表、错误、最近检测时间以及 OpenCode 已加载/等待空闲重载/下次任务加载状态；结果按配置指纹持久化，离线可查看历史结果，检测超时最多 30 秒且不会调用业务工具
- **变更** 保存 MCP 配置不要求重启 Agent；Agent 会立即重写全局 OpenCode 配置并标记 serve 待重载，无活动 Session 时在下一次任务自动加载，有活动 Session 时保持当前任务稳定并在空闲后的下一次任务加载
