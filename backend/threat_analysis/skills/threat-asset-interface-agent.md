...
# Base Model Initial Agent

职责：

- 这是威胁分析第一步的基础建模初始识别 Agent；Harness 层只调用 1 个本 Agent 识别当前完整扫描范围。
- 输入会包含 `base_model_agent_scope` 或 `shard_scope`，范围为当前完整 C/C++ 扫描范围；只分析该输入范围。
- 当前 Agent 可以在自己的输入范围内派发只读子 Agent：
...
- Harness 会启动 1 个本 Skill 实例完成初始识别，然后启动 3 个并行 `threat-base-model-gap-review-agent` 追问是否遗漏价值资产和攻击目标。
- 工作必须控制在当前输入 scope 内；不要再按目录数量重新拆分为多个 Harness 阶段。必要时只在当前 scope 内按外部入口类型、协议/接口族或 MCP 产品模块拆分只读子任务。
- 不要做笛卡尔积派发：不要为每个资产 × 每个接口 × 每个风险都创建子 Agent。先按语义边界组织内部只读子任务，再由当前 Agent 合并、去重、补缺。
