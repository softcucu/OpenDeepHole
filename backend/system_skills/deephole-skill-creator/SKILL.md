...
- The SKILL must instruct the auditor to inspect the target code actively and identify concrete file/function/line evidence.
- If multiple real issues are found, each issue must be returned as a separate element in the final JSON `results` array.
- If no real issue is found, the auditor must still return one `results` element with `confirmed=false`.
...
- Exact final JSON expectations for `confirmed`, `severity`, `description`, `ai_analysis`, `file`, `line`, and `function`.
